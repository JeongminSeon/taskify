...
export const useDashBoardStore = create<DashboardStore>((set, get) => ({
  ...
  fetchColumns: async (dashboardId: number) => {
    set({ loading: true });
    try {
      const response: ColumnsResponse = await getColumns({ dashboardId });
  ...
  addColumn: async (title: string, dashboardId: number) => {
    try {
      const newColumn = await createColumn({ title, dashboardId });
      if (newColumn) {
        set((state) => ({
          columns: [...state.columns, { ...newColumn, dashboardId }],
        }));
      }
